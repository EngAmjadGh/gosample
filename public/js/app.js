    ...
    Array.from(flatpickrExamples).forEach(function (item) {
      if (item.getAttribute("data-provider") == "flatpickr") {
        ...
        if (isTimepickerVal["data-min-time"]) {
          timeData.enableTime = true, timeData.noCalendar = true, timeData.dateFormat = "H:i", timeData.minTime = isTimepickerVal["data-min-time"].value.toString();
        }
        if (isTimepickerVal["data-max-time"]) {
          timeData.enableTime = true, timeData.noCalendar = true, timeData.dateFormat = "H:i", timeData.minTime = isTimepickerVal["data-max-time"].value.toString();
        }
        if (isTimepickerVal["data-default-time"]) {
          timeData.enableTime = true, timeData.noCalendar = true, timeData.dateFormat = "H:i", timeData.defaultDate = isTimepickerVal["data-default-time"].value.toString();
        }
  ...
  function twoColumnMenuGenerate() {
    ...
    if (defaultValues && (isTwoColumn == "twocolumn" || defaultValues["data-layout"] == "twocolumn")) {
      ...
      Array.from(document.querySelector("#two-column-menu ul").querySelectorAll("li a")).forEach(function (element) {
        ...
        currentPath = currentPath.substring(currentPath.lastIndexOf("/") + 1);
        element.addEventListener("click", function (e) {
          if (!(currentPath == "/" + element.getAttribute("href") && !element.getAttribute("data-bs-toggle"))) document.body.classList.contains("twocolumn-panel") ? document.body.classList.remove("twocolumn-panel") : "";
          ...
          if (e.target && e.target.matches("a.nav-icon") || e.target && e.target.matches("i")) {
            if (document.querySelector("#two-column-menu ul .nav-icon.active") !== null) document.querySelector("#two-column-menu ul .nav-icon.active").classList.remove("active");
            ...
            if (document.getElementById(childMenusId)) document.getElementById(childMenusId).parentElement.classList.add("twocolumn-item-show");
          }
        });
    ...
    if (verticalOverlay) {
      Array.from(verticalOverlay).forEach(function (element) {
        element.addEventListener("click", function () {
          ...
          if (sessionStorage.getItem("data-layout") == "twocolumn") document.body.classList.add("twocolumn-panel");else document.documentElement.setAttribute("data-sidebar-size", sessionStorage.getItem("data-sidebar-size"));
        });
      });
    }
  }
      ...
      if (a) {
        ...
        if (parentCollapseDiv && parentCollapseDiv.parentElement.closest(".collapse.menu-dropdown")) {
          ...
          if (parentCollapseDiv.parentElement.parentElement.parentElement.parentElement.closest(".collapse.menu-dropdown")) {
            ...
            if (document.getElementById("two-column-menu").querySelector('[href="#' + menuIdSub + '"]')) document.getElementById("two-column-menu").querySelector('[href="#' + menuIdSub + '"]').classList.add("active");
          }
          var menuId = parentCollapseDiv.parentElement.closest(".collapse.menu-dropdown").getAttribute("id");
          if (document.getElementById("two-column-menu").querySelector('[href="#' + menuId + '"]')) document.getElementById("two-column-menu").querySelector('[href="#' + menuId + '"]').classList.add("active");
        } else {
  ...
  function initModeSetting() {
    ...
    if (lightDarkBtn && lightDarkBtn.length) {
      lightDarkBtn[0].addEventListener("click", function (event) {
        html.hasAttribute("data-layout-mode") && html.getAttribute("data-layout-mode") == "dark" ? setLayoutMode("data-layout-mode", "light", "layout-mode-light", html) : setLayoutMode("data-layout-mode", "dark", "layout-mode-dark", html);
      });
    }
  }
